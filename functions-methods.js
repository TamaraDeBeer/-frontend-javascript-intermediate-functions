...
function checkEmailValidity(emailAddress) {
    ...
    const indexStart = emailAddress.includes("@");
    const lastDotIndex = emailAddress.lastIndexOf(".");
    const commaIndex = emailAddress.includes(",");

    if (!indexStart) {
        return false;
    } else if (commaIndex) {
        return false;
    } else return !(lastDotIndex === -1 || lastDotIndex === emailAddress.length - 1);
}
